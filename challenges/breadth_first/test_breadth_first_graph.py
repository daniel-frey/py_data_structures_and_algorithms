...
def test_graph_has_vertex(graph_filled):
    assert graph_filled.has_vert('D')


def test_graph_doesnt_have_vertex(graph_filled):
    assert not graph_filled.has_vert('Z')


def test_empty_graph_doesnt_have_vertex(graph_empty):
...
def test_returns_graph_length(graph_filled):
    assert len(graph_filled) == 6


def test_returns_length_of_empty_graph(graph_empty):
    assert len(graph_empty) == 0


def test_traversal_returns_the_correct_values(graph_filled):
...
def test_breaks_if_no_node_given(graph_empty):
    assert graph_empty.breadth_first_traversal('A') == []